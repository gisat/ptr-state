      ...
      return (0, _request["default"])(localConfig, apiPath, 'POST', null, payload).then(function (result) {
        if (result.errors && result.errors[dataType] || result.data && !result.data[dataType]) {
          ...
          items.forEach(function (item) {
            ...
            _selectors["default"].getIndexesByFilteredItem(getSubstate)(getState(), item)) || [];
          });
  ...
  return function (dispatch, getState) {
    ...
    if (data.length) {
      ...
      data.forEach(function (model) {
        ...
        indexes = indexes.concat(_selectors["default"].getIndexesByFilteredItem(getSubstate)(getState() || [], model)); //Find corresponding indexes for original model
