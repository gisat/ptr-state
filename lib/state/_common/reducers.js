  ...
  addIndex: function addIndex(state, action) {
    if (action !== null && action !== void 0 && action.data) {
      ...
      if (indexOfIndex > -1) {
        ...
        var updatedIndexIndex = _helpers["default"].registerModelsToIndex(_objectSpread({}, updatedIndex.index), action.data, action.start); // Remove loading indicator if data on this position doesn't exist
