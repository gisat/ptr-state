...
var receiveIndexed = function receiveIndexed(spatialData, filter, order, changedOn) {
  return function (dispatch) {
    if (spatialData && !_lodash["default"].isEmpty(spatialData)) {
...
function addDataAndIndex(spatialDataAndIndexByDataSourceKey, filter, order, changedOn) {
  return function (dispatch) {
    ...
    _lodash["default"].forIn(spatialDataAndIndexByDataSourceKey, function (value, spatialDataSourceKey) {
      spatialDataByDataSourceKey[spatialDataSourceKey] = value.data;
    });

    dispatch(addDataAndIndexAction(spatialDataByDataSourceKey, level, filter, order, [indexByLevelByTileByDataSourceKey], changedOn));
  };
}
/**
...
function addDataAndIndexAction(dataByDataSourceKey, level, filter, order, indexData, changedOn) {
  return {
