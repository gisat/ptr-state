...
function updateComponent(componentKey, update) {
  return function (dispatch, getState) {
    ...
    dispatch(actionAddOrReplaceComponents(_defineProperty({}, componentKey, (0, _lodash.merge)(componentState, update))));
  };
}
/**
...
function loadMissingRelationsAndData(componentKey, order, commonFilter, attributeDataFilterExtension, remainingRelationsPages, remainingAttributeDataPages, start, length, PAGE_SIZE) {
  return function (dispatch, getState) {
    ...
    for (var i = 1; i <= remainingRelationsPages.length; i++) {
      ...
      promises.push(dispatch(loadIndexedPage(order, commonFilter, attributeDataFilterExtension, loadRelations, loadData, relationsPagination, attributePagination)));
...
var ensure = function ensure(componentKey) {
  return function (dispatch, getState) {
    ...
    if (componentState) {
      ...
      if (!(0, _lodash.isEmpty)(attributeDataIndex)) {
      ...
      if (!(0, _lodash.isEmpty)(attributeDataIndex) && !(0, _lodash.isEmpty)(attributeRelationsIndex)) {
  ...
  return function (dispatch) {
    if (result.attributeData || result.attributeRelationsDataSources) {
      ...
      if (result.attributeData.attributeData) {
        var _changes = null;
        dispatch(_actions3["default"].receiveIndexed(result.attributeData, attributeDataFilter, order, result.attributeData.offset + 1, result.attributeData.total, _changes));
      }
    }
  };
};
...
var actionAddOrReplaceComponents = function actionAddOrReplaceComponents(components) {
  return {
    type: _ActionTypes["default"].DATA.COMPONENTS.ADD_COMPONENTS,
