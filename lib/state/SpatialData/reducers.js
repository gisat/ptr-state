...
var add = function add(state, action) {
  ...
  if (action.data && action.data.length) {
    action.data.forEach(function (model) {
      newData[model.spatialDataSourceKey] = _objectSpread(_objectSpread({}, newData[model.spatialDataSourceKey]), model);
    });
  }
...
var _default = function _default() {
  ...
  switch (action.type) {
    case _ActionTypes["default"].SPATIAL_DATA.ADD:
      return add(state, action);

    case _ActionTypes["default"].SPATIAL_DATA.INDEX.ADD:
      return addIndex(state, action);

    case _ActionTypes["default"].SPATIAL_DATA.USE.INDEXED.REGISTER:
