...
var INITIAL_STATE = _objectSpread({}, _reducers.DEFAULT_INITIAL_STATE); // Add should be actually update, because dataSourceKey is used as key
  ...
  if (action.data && action.data.length) {
    action.data.forEach(function (model) {
      ...
      if (previousData) {
        ...
        _lodash["default"].forEach(previousFeatures, function (feature) {
          newFeaturesAsObject[feature.properties[fidColumnName]] = feature;
        });

        _lodash["default"].forEach(nextFeatures, function (feature) {
          var key = feature.properties[fidColumnName];
          newFeaturesAsObject[key] = feature;
        });

        newData[model.attributeDataSourceKey] = _objectSpread(_objectSpread({}, newData[model.attributeDataSourceKey]), {}, {
          attributeData: _objectSpread(_objectSpread({}, newData[model.attributeDataSourceKey].attributeData), {}, {
...
var _default = function _default() {
  ...
  switch (action.type) {
    case _ActionTypes["default"].ATTRIBUTE_DATA.ADD:
      return update(state, action);

    case _ActionTypes["default"].ATTRIBUTE_DATA.ADD_BATCH:
      return _reducers["default"].addBatch(state, action);

    case _ActionTypes["default"].ATTRIBUTE_DATA.ADD_UNRECEIVED:
      return _reducers["default"].addUnreceivedKeys(state, action);

    case _ActionTypes["default"].ATTRIBUTE_DATA.INDEX.ADD:
      return _reducers["default"].addIndex(state, action);

    case _ActionTypes["default"].ATTRIBUTE_DATA.INDEX.ADD_BATCH:
      return _reducers["default"].addBatchIndex(state, action);

    case _ActionTypes["default"].ATTRIBUTE_DATA.USE.INDEXED.REGISTER:
      return _reducers["default"].registerUseIndexed(state, action);

    case _ActionTypes["default"].ATTRIBUTE_DATA.USE.INDEXED_BATCH.REGISTER:
      return _reducers["default"].registerBatchUseIndexed(state, action);

    case _ActionTypes["default"].ATTRIBUTE_DATA.USE.INDEXED.CLEAR:
