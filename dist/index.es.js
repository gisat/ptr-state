...
import { utils, CacheFifo, map, stateManagement } from '@gisatcz/ptr-utils';
...
import _reduce2 from 'lodash/reduce';
import _isNumber2 from 'lodash/isNumber';
import _head2 from 'lodash/head';
import createCachedSelector from 're-reselect';
import _pickBy2 from 'lodash/pickBy';
import _pick2 from 'lodash/pick';
import _difference2 from 'lodash/difference';
...
var getCompleteLocalConfigurationObserver = createObserver(getCompleteLocalConfiguration);
...
function removeIndex() {
  ...
  if (indexes && !_isEmpty2(indexes)) {
    var clearedIndexes = _reduce2(indexes, function (acc, index) {
...
function isNaturalNumber(input) {
  return _isNumber2(input) && input > 0 && input % 1 === 0;
}
/**
...
var haveAllKeysRegisteredUse = function haveAllKeysRegisteredUse(getSubstate) {
  return createCachedSelector([getUsedKeysForComponent(getSubstate), function (state, componentKey, keys) {
    ...
    if (usedKeys && keys !== null && keys !== void 0 && keys.length) {
      var notIncluded = _difference2(keys, usedKeys);
...
var getUsedIndexPage = function getUsedIndexPage(getSubstate) {
  return createCachedSelector(getUsedIndexPages(getSubstate), function (state, filter) {
    ...
    var stringFilter = JSON.stringify(_map2(filter, function (value, key) {
...
var getUsesWithActiveDependency = function getUsesWithActiveDependency(getSubstate) {
  ...
  function (indexedDataUses, activeKeys, filterByActive) {
    ...
    if (filterByActive && !_isEmpty2(indexedDataUses)) {
      ...
      _map2(indexedDataUses, function (componentUsedIndexes) {
        // loop through uses for component
        _map2(componentUsedIndexes, function (usedIndex) {
          if (_reduce2(filterByActive, function (accumulator, value, index) {
      ...
      if (!_isEmpty2(groupedUses)) {
        _map2(groupedUses, function (groupedUse) {
...
var getAllData = createSelector([getAll$3], function (relations) {
  if (relations) {
    return _map2(relations, function (relation) {
...
var getFilteredDataSourceKeysGroupedByLayerKey = createCachedSelector([getAll$3, function (state, layers) {
  ...
  if (relations && relations.length) {
    ...
    _forEach2(layers, function (layer) {
...
var getFilteredAttributeDataSourceKeyAttributeKeyPairs = createSelector$1(function (filter) {
  ...
  if (relations) {
    ...
    _forEach2(relations, function (relation) {
...
var getDataByDataSourceKeys = createSelector$1(function (dataSourceKeys) {
  if (dataSourceKeys) {
    ...
    _forEach2(dataSourceKeys, function (key) {
...
var getAttributesByDataSourceKeysForFeatureKey = createSelector$1(function (attributeDataSourceKeyAttributeKeyPairs, featureKey) {
  if (attributeDataSourceKeyAttributeKeyPairs && featureKey) {
    ...
    if (dataByDataSourceKey) {
      ...
      _forIn2(dataByDataSourceKey, function (dataSourceData, dataSourceKey) {
            ...
            var _ret2 = function () {
              ...
              var _loop = function _loop(i) {
                ...
                if (featureKey) {
                  ...
                  _forIn2(attributeDsKeyAttributeKeyPairs, function (attributeKey, attributeDsKey) {
                    ...
                    if (value !== undefined) {
                      ...
                      if (finalFeaturesAsObject[i - start]) {
                        finalFeaturesAsObject[i - start].data[attributeKey] = value;
                          ...
                          value = isNaN(value) ? value : _isNumber2(value) ? value : Number(value); // TODO format?
...
var getIndexedFeatureKeys = createSelector$1(function (filter, level, tile, dataSourceKey) {
  if (_isNumber2(level) && tile && dataSourceKey) {
...
var getMissingTiles = function getMissingTiles(index, filter) {
  if (index && index.index && filter && _isArray2(filter.tiles)) {
    ...
    if ((_index$index = index.index) !== null && _index$index !== void 0 && _index$index[filter.level]) {
      var loadedTilesInIndex = _reduce2(index.index[filter.level], function (acc, tileData, tileKey) {
...
var getFeatures = createSelector$1(function (dataSourceKey, fidColumnName, attributeDataSourceKeyAttributeKeyPairs) {
  ...
  if (data) {
    return _map2(data, function (feature, key) {
      var properties = _defineProperty({}, fidColumnName, key);

      if (attributesByDataSourceKey) {
        _forIn2(attributesByDataSourceKey, function (features, attributeDataSourceKey) {
...
var getAllAsObjectWithStyles = createSelector([getAllAsObject$a, Styles.getAllAsObject], function (selections, allStyles) {
  if (selections) {
    if (allStyles) {
      ...
      _forIn2(selections, function (selection, key) {
        var _selection$data;
...
var prepareSelectionByLayerStateSelected = createSelector$1(function (layerStateSelected) {
  ...
  _forIn2(layerStateSelected, function (value, key) {
...
var getActiveMap = createSelector([getMapsAsObject, getActiveMapKey], function (maps, activeMapKey) {
...
var getMapLayerStateByMapKeyAndLayerKey = createSelector([getMapLayersStateByMapKey, function (state, mapKey, layerKey) {
  return layerKey;
...
var getVisibleTilesByMapKey = createCachedSelector([getViewByMapKey, function (state, mapKey, mapWidth) {
  return mapWidth;
...
var getMapLayers = createSelector$1(function (mapKey, layersState) {
  ...
  if (layersState) {
    ...
    _forEach2(layersState, function (layerState) {
      ...
      if (layerState.type) {
        ...
        finalLayers.push(layerState);
          ...
          if (spatialDataSources) {
            _forEach2(spatialDataSources, function (dataSource) {
...
var getAsObjectByFullPeriod = createCachedSelector([getAllAsObject$b, function (state, limitStart) {
  return limitStart;
  ...
  if (periods && limitStart && limitEnd) {
    var selectedPeriods = _pickBy2(periods, function (period) {
      ...
      if (periodStart && periodEnd) {
        return moment(periodStart).isBetween(limitStart, limitEnd, null, '[]') && moment(periodEnd).isBetween(limitStart, limitEnd, null, '[]');
      } else if (periodStart) {
        return moment(periodStart).isBetween(limitStart, limitEnd, null, '[]');
      } else if (periodEnd) {
...
function (sets, lineage) {
  ...
  _forIn2(sets, function (value, key) {
  ...
  return function (dispatch, getState) {
    ...
    if (data.length) {
      ...
      data.forEach(function (model) {
        ...
        _forIn2(edited, function (value, key) {
  ...
  return function () {
    return function (dispatch, getState) {
      ...
      var promises = _flatMap(usedIndexPages, function (usedIndexPage) {
        _map2(usedIndexPage.uses, function (use) {
  ...
  return function (filterByActive) {
    return function (dispatch, getState) {
      ...
      var promises = _flatMap(usedIndexes, function (usedIndex) {
        _map2(usedIndex.uses, function (use) {
...
function addLoadingSpatialIndex(attributeDataFilter, order, level, tiles) {
  ...
  var loadingTiles = _reduce2(tiles, function (acc, tile) {
...
function getRestPages(count, PAGE_SIZE) {
  ...
  optStart = _isNumber2(optStart) ? optStart : 1;

  if (_isNumber2(count) && (count === 0 || optStart > count)) {
    return [];
  } else {
    var wanted;
...
function getPagination(pageIndex) {
  ...
  if (_isNumber2(optLength) && pageIndex * pageSize + pageSize > optLength) {
    limit = Math.max(0, optLength - pageIndex * pageSize);
  }

  if (_isNumber2(optCount) && optStart + pageIndex * pageSize + limit - 1 > optCount) {
...
function getLoadedPages() {
  ...
  pages.forEach(function (pageIndex) {
    ...
    if (_isNumber2(optCount)) {
      if (_isNumber2(optLength) && pageSize * (pageIndex + 1) > optLength) {
...
function getMissingPages(optDataIndex, pageSize, optStart, optLength) {
  ...
  var missingPages = _difference2(restPages, loadedPages);
...
function updateComponent(componentKey, update) {
  return function (dispatch, getState) {
    ...
    dispatch(actionAddOrReplaceComponents(_defineProperty({}, componentKey, _merge2(componentState, update))));
  };
}
/**
...
function addDataAndIndex(spatialDataAndIndexByDataSourceKey, filter, order, changedOn) {
  return function (dispatch) {
    ...
    _forIn2(spatialDataAndIndexByDataSourceKey, function (value, spatialDataSourceKey) {
  ...
  return function (dispatch, getState) {
    ...
    if (promises.length > 0) {
      return Promise.all(promises).then(function () {
        ...
        var spatialDataSourcesTypes = _flattenDeep2(response.map(function (r) {
...
function ensure$1(styleKey, commonRelationsFilter, spatialFilter, attributeDataFilterExtension) {
  return function (dispatch, getState) {
    ...
    if (!loadRelationsAndData && missingAttributesData) {
      ...
      var missingAttributeDataTilesToLoad = _difference2(missingAttributeDataTiles, missingSpatialDataTiles);

      var missingSpatialAndAttributeDataTiles = _intersection2(missingAttributeDataTiles, missingSpatialDataTiles);
...
function setLoading(attributeDataFilter, spatialIndex, spatialFilter, spatialRelationsFilter, order, loadGeometry) {
  return function (dispatch, getState) {
    ...
    var spatialTilesInNotLoadingState = _reduce2(loadingTilesGeometry, function () {
    ...
    var attributesTilesInNotLoadingState = _reduce2(loadingTilesGeometry, function () {
      var acc = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
      var tile = arguments.length > 1 ? arguments[1] : undefined;
...
function setMapViewport(mapKey, width, height) {
  return function (dispatch, getState) {
    if (mapKey && _isNumber2(width) && _isNumber2(height)) {
      var state = getState();
...
var commonReducers = {
  ...
  clearIndexes: function clearIndexes(state, action) {
    ...
    if ((_state$indexes = state.indexes) !== null && _state$indexes !== void 0 && _state$indexes.length) {
      var indexes = _map2(state.indexes, function (index) {
...
function ensureDependenciesOfActiveMetadataType(filterKey) {
  return function (dispatch) {
    ...
    _forIn2(STORES_TO_ENSURE_WITH_FILTER_BY_ACTIVE, function (params) {
...
var componentUseClear$1 = function componentUseClear(state, componentKey) {
  if (componentKey && !_isEmpty2(state.components.inUse)) {
    ...
    if (index > -1) {
      var updatedInUse = stateManagement.removeItemByIndex(state.components.inUse, index);
      return _objectSpread2(_objectSpread2({}, state), {}, {
...
function getUpdatedByDataSourceKey$1(state, dataByDataSourceKey, level) {
  ...
  _forIn2(dataByDataSourceKey, function (data, dataSourceKey) {
    ...
    _forIn2(data, function (geometry, featureKey) {
...
var removeMapFromSet$1 = function removeMapFromSet(state, setKey, mapKey) {
  if (setKey && mapKey) {
    ...
    if (index > -1) {
      var updatedMaps = stateManagement.removeItemByIndex(state.sets[setKey].maps, index);
      return _objectSpread2(_objectSpread2({}, state), {}, {
...
var removeMapLayer$1 = function removeMapLayer(state, mapKey, layerKey) {
  if (layerKey && mapKey) {
    ...
    var index = _findIndex2((_state$maps$mapKey = state.maps[mapKey]) === null || _state$maps$mapKey === void 0 ? void 0 : _state$maps$mapKey.data.layers, {
      key: layerKey
    });
    ...
    if (index > -1) {
      ...
      var updatedLayers = stateManagement.removeItemByIndex((_state$maps$mapKey2 = state.maps[mapKey]) === null || _state$maps$mapKey2 === void 0 ? void 0 : _state$maps$mapKey2.data.layers, index);
      return _objectSpread2(_objectSpread2({}, state), {}, {
  ...
  if (mapKey && layerState && (_state$maps2 = state.maps) !== null && _state$maps2 !== void 0 && _state$maps2[mapKey]) {
    ...
    if (currentLayers) {
      if (index > -1 && index < currentLayers.length) {
        updatedLayers = stateManagement.addItemToIndex(currentLayers, index, layerState);
      } else {
...
var mapUseClear$1 = function mapUseClear(state, mapKey) {
  if (mapKey) {
    ...
    if (index > -1) {
      var updatedInUse = stateManagement.removeItemByIndex(state.inUse.maps, index);
      return _objectSpread2(_objectSpread2({}, state), {}, {
        inUse: _objectSpread2(_objectSpread2({}, state.inUse), {}, {
          maps: updatedInUse
        })
...
var mapSetUseClear$1 = function mapSetUseClear(state, mapSetKey) {
  if (mapSetKey) {
    ...
    if (index > -1) {
      var updatedInUse = stateManagement.removeItemByIndex(state.inUse.sets, index);
      return _objectSpread2(_objectSpread2({}, state), {}, {
        inUse: _objectSpread2(_objectSpread2({}, state.inUse), {}, {
          sets: updatedInUse
        })
...
var createBaseStore = function createBaseStore(specificStores) {
  ...
  if (preloadedState) {
    return createStore(combineReducers(stores), preloadedState, compose(reduxBatch, appliedMiddleware, reduxBatch));
  } else {
    return createStore(combineReducers(stores), compose(reduxBatch, appliedMiddleware, reduxBatch));
  }
};
